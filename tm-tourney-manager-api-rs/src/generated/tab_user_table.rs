...
/// Access to the `account_id` unique index on the table `tab_user`,
/// which allows point queries on the field of the same name
/// via the [`TabUserAccountIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.tab_user().account_id().find(...)`.
pub struct TabUserAccountIdUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<User, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> TabUserTableHandle<'ctx> {
    /// Get a handle on the `account_id` unique index on the table `tab_user`.
    pub fn account_id(&self) -> TabUserAccountIdUnique<'ctx> {
        TabUserAccountIdUnique {
...
impl<'ctx> TabUserAccountIdUnique<'ctx> {
