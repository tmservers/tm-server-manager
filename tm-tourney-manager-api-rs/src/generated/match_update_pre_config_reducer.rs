// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

use super::server_config_type::ServerConfig;

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub(super) struct MatchUpdatePreConfigArgs {
    pub id: u32,
    pub config: ServerConfig,
}

impl From<MatchUpdatePreConfigArgs> for super::Reducer {
    fn from(args: MatchUpdatePreConfigArgs) -> Self {
        Self::MatchUpdatePreConfig {
            id: args.id,
            config: args.config,
        }
    }
}

impl __sdk::InModule for MatchUpdatePreConfigArgs {
    type Module = super::RemoteModule;
}

pub struct MatchUpdatePreConfigCallbackId(__sdk::CallbackId);

#[allow(non_camel_case_types)]
/// Extension trait for access to the reducer `match_update_pre_config`.
///
/// Implemented for [`super::RemoteReducers`].
pub trait match_update_pre_config {
    /// Request that the remote module invoke the reducer `match_update_pre_config` to run as soon as possible.
    ///
    /// This method returns immediately, and errors only if we are unable to send the request.
    /// The reducer will run asynchronously in the future,
    ///  and its status can be observed by listening for [`Self::on_match_update_pre_config`] callbacks.
    fn match_update_pre_config(&self, id: u32, config: ServerConfig) -> __sdk::Result<()>;
    /// Register a callback to run whenever we are notified of an invocation of the reducer `match_update_pre_config`.
    ///
    /// Callbacks should inspect the [`__sdk::ReducerEvent`] contained in the [`super::ReducerEventContext`]
    /// to determine the reducer's status.
    ///
    /// The returned [`MatchUpdatePreConfigCallbackId`] can be passed to [`Self::remove_on_match_update_pre_config`]
    /// to cancel the callback.
    fn on_match_update_pre_config(
        &self,
        callback: impl FnMut(&super::ReducerEventContext, &u32, &ServerConfig) + Send + 'static,
    ) -> MatchUpdatePreConfigCallbackId;
    /// Cancel a callback previously registered by [`Self::on_match_update_pre_config`],
    /// causing it not to run in the future.
    fn remove_on_match_update_pre_config(&self, callback: MatchUpdatePreConfigCallbackId);
}

impl match_update_pre_config for super::RemoteReducers {
    fn match_update_pre_config(&self, id: u32, config: ServerConfig) -> __sdk::Result<()> {
        self.imp.call_reducer(
            "match_update_pre_config",
            MatchUpdatePreConfigArgs { id, config },
        )
    }
    fn on_match_update_pre_config(
        &self,
        mut callback: impl FnMut(&super::ReducerEventContext, &u32, &ServerConfig) + Send + 'static,
    ) -> MatchUpdatePreConfigCallbackId {
        MatchUpdatePreConfigCallbackId(self.imp.on_reducer(
            "match_update_pre_config",
            Box::new(move |ctx: &super::ReducerEventContext| {
                #[allow(irrefutable_let_patterns)]
                let super::ReducerEventContext {
                    event:
                        __sdk::ReducerEvent {
                            reducer: super::Reducer::MatchUpdatePreConfig { id, config },
                            ..
                        },
                    ..
                } = ctx
                else {
                    unreachable!()
                };
                callback(ctx, id, config)
            }),
        ))
    }
    fn remove_on_match_update_pre_config(&self, callback: MatchUpdatePreConfigCallbackId) {
        self.imp
            .remove_on_reducer("match_update_pre_config", callback.0)
    }
}

#[allow(non_camel_case_types)]
#[doc(hidden)]
/// Extension trait for setting the call-flags for the reducer `match_update_pre_config`.
///
/// Implemented for [`super::SetReducerFlags`].
///
/// This type is currently unstable and may be removed without a major version bump.
pub trait set_flags_for_match_update_pre_config {
    /// Set the call-reducer flags for the reducer `match_update_pre_config` to `flags`.
    ///
    /// This type is currently unstable and may be removed without a major version bump.
    fn match_update_pre_config(&self, flags: __ws::CallReducerFlags);
}

impl set_flags_for_match_update_pre_config for super::SetReducerFlags {
    fn match_update_pre_config(&self, flags: __ws::CallReducerFlags) {
        self.imp
            .set_call_reducer_flags("match_update_pre_config", flags);
    }
}
