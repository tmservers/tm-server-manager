...
///
/// Obtain a handle from the [`MatchLeaderboardTableAccess::match_leaderboard`] method on [`super::RemoteTables`],
/// like `ctx.db.match_leaderboard()`.
///
...
/// Extension trait for access to the table `match_leaderboard`.
///
/// Implemented for [`super::RemoteTables`].
pub trait MatchLeaderboardTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`MatchLeaderboardTableHandle`], which mediates access to the table `match_leaderboard`.
...
pub struct MatchLeaderboardInsertCallbackId(__sdk::CallbackId);
...
impl<'ctx> __sdk::Table for MatchLeaderboardTableHandle<'ctx> {
    ...
    type InsertCallbackId = MatchLeaderboardInsertCallbackId;

    fn on_insert(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> MatchLeaderboardInsertCallbackId {
        MatchLeaderboardInsertCallbackId(self.imp.on_insert(Box::new(callback)))
    }

    fn remove_on_insert(&self, callback: MatchLeaderboardInsertCallbackId) {
        self.imp.remove_on_insert(callback.0)
    }

    type DeleteCallbackId = MatchLeaderboardDeleteCallbackId;

    fn on_delete(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> MatchLeaderboardDeleteCallbackId {
        MatchLeaderboardDeleteCallbackId(self.imp.on_delete(Box::new(callback)))
    }

    fn remove_on_delete(&self, callback: MatchLeaderboardDeleteCallbackId) {
